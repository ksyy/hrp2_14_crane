...
void CraneMessageHandler::positionControlStrategy()
{
  ...
  for(unsigned int i=0;i<DIM_TO_CONTROL;i++)
    {
      error[i] = desired_position_[i] - encoder_values_[i+1];
  ...
  if ((intervalcontrol>0.07) && 
      (intervalreadingandnow>0.02))
    {
      /*
...
void CraneMessageHandler::applyControlStrategy()
{
  ...
  char valid_encoders=true;
  int r = 0;
  
  struct timeval reading;
  gettimeofday(&reading,0);

  double intervalreading = computeTime(lastreading_,
				       reading);

  double intervalcontrolandnow  = computeTime(lastcontrol_,
				       reading);
  if ((intervalreading>0.07) && 
      (intervalcontrolandnow>0.02))
    {
      r=ptp_read_encoders(encoder_values_);
  ...
  switch(strategy_)
    {
    case JOYSTICK:joystickStrategy();
      break;
    case POSITION:
      if (valid_encoders)
  ...
  if (intervalreading>0.07)
    if (valid_encoders)
      std::cout << intervalreading << " Positions:" << encoder_values_[1] 
		<<" " << encoder_values_[2] << std::endl;
}
